...
@function_tool(
    description=(
        ...
        "that feed the negotiation's two timing ladders: today + the 3-day "
        "date + the 7-day date, for an initial/partial payment; the 3-day "
        "date + the 7-day date + the 30-day date, for a full payment. Your "
        "instructions already give you today's date and all three deadline "
        "dates at the start of this call — treat those as authoritative and "
        "do not call this tool to re-derive them. Only call this if the "
        "customer disputes today's date, the call has been running for an "
        "unusually long time, or you need to convert something the customer "
        "says ('this Friday', 'tomorrow') into an exact calendar date. Never "
        "guess, assume, do your own date arithmetic, or rely on your "
        "training data for today's date — call this instead, and always "
...
async def get_current_datetime(ctx: RunContext) -> str:
    ...
    return (
        ...
        f"Today's date, for a same-day payment ask: "
        ...
        f"30-day payment deadline, i.e. 1 month out (speak this exact date, "
        f"never 'in 30 days', 'in a month', or 'end of month'): "
        f"{thirty_day_deadline.strftime('%A, %B %d, %Y')}."
    )
...
async def entrypoint(ctx: JobContext):
    ...
    system_prompt += (
        ...
        f"{_call_start_dt.strftime('%A, %B %d, %Y')} — this is also the "
        f"same-day date for a same-day payment ask. The 3-day payment "
        ...
        f"The 30-day payment deadline (1 month out) is "
        f"{_thirty_day_deadline_dt.strftime('%A, %B %d, %Y')}. Per Part 5, "
        f"today/3-day/7-day are the timing ladder for an initial or partial "
        f"payment; 3-day/7-day/30-day are the timing ladder for a full "
        f"payment. These are fixed for the rest of this call — speak them "
        f"exactly as given here, never as 'today or tomorrow', 'in 3 days', "
        f"'in 7 days', 'this week', 'in 30 days', 'in a month', or 'end of "
    ...
    tts = agents_tts.FallbackAdapter(
        [
            inference.TTS(
                ...
                voice="v_ckr9NXBNDLXy",#"v_VeRxYTHdQqGg",#"v_XSvqo8UVEFYo","v_tkbNkcSD62zN",#"v_ebJJAf8QhLMs",
                extra_kwargs={"speed": 1.30, "temperature": 0, "latency": "normal"},
            ),

            inference.TTS(
                model="fishaudio/s2-pro",
                voice="v_ckr9NXBNDLXy",#"v_ckr9NXBNDLXy",#"v_XSvqo8UVEFYo","v_tkbNkcSD62zN",#"v_ebJJAf8QhLMs",
                extra_kwargs={"speed": 1.30, "temperature": 0, "latency": "normal"},
            ),
